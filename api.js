...
app.post('/schedule', async (req, res) => {
    ...
    console.log('Received a scheduled message request.')
    try {
      // Parse the request body to get the scheduled message details
      const { scheduledMessage, scheduledDateTime, channelId, interval, customIntervalMinutes } = req.body;
  
      // Convert the scheduled date and time to a moment object
      ...
      res.status(200).json({ message: 'Scheduled message saved.' });
    } catch (error) {
    ...
    if (message.content.startsWith("!schedule")) {
      ...
      if (!scheduledMessage) {
        return message.reply(
          "Please provide the message for the scheduled task."
        );
      }

      if (!scheduledDateTime) {
        return message.reply(
          "Please provide the date and time for the scheduled task in the format: YYYY-MM-DD HH:MM."
        );
      }

      if (!mentionedChannel) {
        return message.reply(
          "Please mention the channel for the scheduled task."
        );
      }

      if (!interval) {
        return message.reply(
          "Please provide the interval for the scheduled task (yearly, monthly, daily, custom)."
        );
      }
      ...
      if (interval === "custom") {
        ...
        if (isNaN(customIntervalMinutes)) {
          return message.reply(
            "Invalid custom interval minutes. Please provide a valid number of minutes"
          );
        }
      }

      const dateTime = moment
        .tz(scheduledDateTime, "YYYY-MM-DD HH:mm", timeZone)
        .utc();

      // const dateTime = moment.utc(scheduledDateTime, 'YYYY-MM-DD HH:mm').local();
      if (!dateTime.isValid()) {
        return message.reply(
          "Invalid date or time format. Please use the format: YYYY-MM-DD HH:MM"
        );
      }
      ...
      message.reply(
        `Scheduled message "${scheduledMessage}" saved for ${scheduledDateTime}`
      );
      ...
      if (!messageId) {
        return message.reply(
          "Please provide the ID of the scheduled message to delete."
        );
      }

      // Find the scheduled message by ID and remove it from the database
      const deletedMessage = await ScheduledMessage.findByIdAndRemove(
        messageId
      );
      ...
      message.reply(
        `Scheduled message "${deletedMessage.content}" with ID "${messageId}" deleted.`
      );
      ...
      const messageList = scheduledMessages
        .map((scheduledMessage) => {
          const dateTime = moment(scheduledMessage.date)
            .tz(timeZone)
    ...
    const dueMessages = await ScheduledMessage.find({
      $or: [
        { date: { $lte: currentDate } },
        {
          ...
          date: {
            $lte: moment()
              .subtract({ minutes: { $mod: ["$customIntervalMinutes", 0] } })
              .toDate(),
          },
        },
      ],
